...
def zip_depends_lib(lib):
  f = zipfile.ZipFile('%s.zip' % lib, 'w')

  for dirpath, dirnames, filenames in os.walk('depends/%s' % lib):
    if dirpath == os.path.join('depends', lib, 'contrib'):
      _strip_contrib(dirnames)
    ...
    for filename in filenames:
      ...
      if not os.path.isfile(name):
        ...
        continue
      logging.debug('Adding %s...', name)
      f.write(name, name[len('depends/%s' % lib):], zipfile.ZIP_DEFLATED)
